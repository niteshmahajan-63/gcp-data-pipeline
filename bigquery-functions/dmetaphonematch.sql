CREATE OR REPLACE FUNCTION `sync_metadata.dmetaphonematch`(str1 string, str2 string)
RETURNS boolean
LANGUAGE js AS """
...
function dmetaphone(str1, str2)
{
	d1 = double_metaphone(str1);
	d2 = double_metaphone(str2);
	if (d1.primary == d2.primary || d1.primary == d2.secondary || d1.secondary == d2.primary || d1.secondary == d2.secondary)
	{
		return true;
	}
	else
