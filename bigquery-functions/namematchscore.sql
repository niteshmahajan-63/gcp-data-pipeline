...
function checkrelations(name) {
    let rel_list = ['baby', 'bo', 'capt', 'wo', 'so', 'do', 'co', 'mo', 'ho', 'fo', 'baby of', 'w o', 'b o', 's o', 'd o', 'c o'];
    let status = true;
...
function nms(aname, bname, aage, bage, agender, bgender, adob, bdob, cyear) {
	...
	let special_cases = ['baby', 'bo', 'capt', 'wo', 'so', 'do', 'co', 'mo', 'ho', 'fo', 'baby of', 'w o', 'b o', 's o', 'd o', 'c o']
	...
	allowed_age = (aage !== null && bage !== null && 
                   (cyear - aage) >= 50 && (cyear - bage) >= 50) ? 15 : 5;
    ...
    if (b_name_list[0] === a_name_list[0] && alen === 2 && blen === 2) {
        if (levenshteinDistance(a_name_list[1], b_name_list[1]) === 1) {
            return 4;
        }
    }
	for (let i = 0; i < alen; i++) {
        ...
        if (levenshteinDistance(a_name_list[i], b_name_list[i]) <= 1 && 
            a_name_list[i].length > 3 && b_name_list[i].length > 3 && 
            (aage === null || bage === null || Math.abs(aage - bage) <= allowed_age)) {
            continue;
        }
        if (levenshteinDistance(a_name_list[i], b_name_list[i]) <= 2 && 
            a_name_list[i].length > 5 && b_name_list[i].length > 5 && 
            (aage === null || bage === null || Math.abs(aage - bage) <= allowed_age)) {
            continue;
        }
    ...
    let maxpos = Math.floor(alen / 2) > 1? Math.floor(alen / 2): 1;
