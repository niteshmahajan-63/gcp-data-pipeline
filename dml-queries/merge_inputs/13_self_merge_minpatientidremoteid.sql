...
-----------------------------------------------------------------------------
drop table if exists `sync_target_merge.minpremoteid`;
create table `sync_target_merge.minpremoteid` cluster by patientlocalid as
with mergeRemoteIds AS (
  select
    ...
    labname
  from
    `sync_target_merge.minpidremoteid`
),
minRemoteId AS(
  select
  patientlocalid,
...
create table `sync_target_merge.merge_input_apid` cluster by remoteid as
...
left join `sync_target_merge.minpremoteid` mpr
...
-----------------------------------------------------------------------------
drop table if exists `sync_target_merge.minmremoteid`;
create table `sync_target_merge.minmremoteid` cluster by patientlocalid as
with mergeRemoteIds AS (
  select
    distinct
    remoteid,
    ...
    labname
  from
    `sync_target_merge.minmobileremoteid`
),
minRemoteId AS(
  select
  patientlocalid,
...
create table `sync_target_merge.merge_input_amid` cluster by remoteid as
...
left join `sync_target_merge.minmremoteid` mmr
...
-----------------------------------------------------------------------------
drop table if exists `sync_target_merge.minamremoteid`;
create table `sync_target_merge.minamremoteid` cluster by patientlocalid as
with mergeRemoteIds AS (
  select
    distinct
    remoteid,
    ...
    labname
  from
    `sync_target_merge.minaltmobileremoteid`
),
minRemoteId AS(
  select
  patientlocalid,
...
create table `sync_target_merge.merge_input_aamid` cluster by remoteid as
select distinct inp.patientid, inp.encounterid, inp.age, inp.gender, inp.dob, inp.maritalstatus, inp.uid, inp.mobile, inp.alternatemobile, inp.email, inp.remarks, inp.address, inp.centreid, inp.city, inp.state, inp.country, inp.nationalityid, inp.pin, inp.insertedon, inp.joiningdate, inp.updateddate, inp.salutation, inp.name, inp.patienthash, inp.deceased, inp.labname, inp.pid, inp.eid, inp.emailid, inp.mobileid, inp.alternatemobileid, COALESCE(mmr.remoteid,inp.remoteid) as remoteid
from `sync_target_merge.merge_input_amid` inp
left join `sync_target_merge.minamremoteid` mmr
on inp.remoteid = mmr.patientlocalid and inp.labname = mmr.labname;
...
-----------------------------------------------------------------------------
drop table if exists `sync_target_merge.mineremoteid`;
create table `sync_target_merge.mineremoteid` cluster by patientlocalid as
with mergeRemoteIds AS (
  select
    distinct
    remoteid,
    ...
    labname
  from
    `sync_target_merge.minemailremoteid`
),
minRemoteId AS(
  select
  patientlocalid,
...
create table `sync_target_merge.merge_input_aeid` cluster by remoteid as
...
left join `sync_target_merge.mineremoteid` mer
...
-----------------------------------------------------------------------------
drop table if exists `sync_target_merge.minuremoteid`;
create table `sync_target_merge.minuremoteid` cluster by patientlocalid as
with mergeRemoteIds AS (
  select
    distinct
    remoteid,
    ...
    labname
  from
    `sync_target_merge.minuidremoteid`
),
minRemoteId AS(
  select
  patientlocalid,
...
create table `sync_target_merge.merge_input_auid` cluster by remoteid as
...
left join `sync_target_merge.minuremoteid` mur
